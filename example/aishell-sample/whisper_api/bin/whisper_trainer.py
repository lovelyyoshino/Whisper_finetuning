...
from whisper_api.tokenizer.toknizer_whisper import WhisperTokenizer
from whisper_api.utils.common_utils import (
    log_info
)
from whisper_api.utils.data_utils import (
...
class WhisperTrainer:
    ...
    def train_start(self):
        ...
        trainer.save_model()
        whisper_model = self.config['model']['model_train_argv']['out_model_path']
        # whisper模型
        log_info("加载 whisper模型")
        model_final = WhisperForConditionalGeneration.from_pretrained(whisper_model)
        
        model_path =self.config['model']['model_train_argv']['out_model_path']+"/tiny.pt"
        self.convert_hf_whisper(whisper_model,model_path)
        log_info("模型已保存为 PT 文件：" + model_path)
    ...
    def rename_keys(self, s_dict):
        ...
        for key in keys:
            new_key = key
            for k, v in self.WHISPER_MAPPING.items():
                if k in key:
        ...
        return s_dict
    # https://github.com/bayartsogt-ya/whisper-multiple-hf-datasets/blob/main/src/multiple_datasets/hub_default_utils.py
    def convert_hf_whisper(self, hf_model_name_or_path: str, whisper_state_path: str):
